 ...
 */
public class  ApacheDrillTruncateOperatorRenderer 
extends BaseOperatorRenderer
{
	static final Logger logger = LoggerFactory.getLogger(ApacheDrillTruncateOperatorRenderer.class);
	
	public  ApacheDrillTruncateOperatorRenderer() {
	}
	...
	public String prettyPrint(SQLSkin skin, OperatorDefinition opDef, String[] args) throws RenderingException {
		...
		if (args.length == 1) {
			//if(logger.isDebugEnabled()){logger.debug(("TRUNCATE(" + args[0] + ")"));}
			return "TRUNCATE(" + args[0] + ")";
		} else {
			//if(logger.isDebugEnabled()){logger.debug(("TRUNCATE(" + args[0] + "," + args[1] + ")"));}
