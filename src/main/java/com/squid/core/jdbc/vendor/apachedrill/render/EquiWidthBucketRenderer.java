...
public class EquiWidthBucketRenderer 
extends BaseOperatorRenderer
{
	static final Logger logger = LoggerFactory.getLogger(EquiWidthBucketRenderer.class);
	...
	public String prettyPrint(SQLSkin skin, OperatorDefinition opDef, String[] args) throws RenderingException {
		...
		if (args != null) {
			if (args.length!=4) throw new RuntimeException("invalid syntax for EquiWidthBucket operator");
			metric = args [0];
			lowerBound = args [1];
			upperBound = args [2];
			bucketCount = args [3];
